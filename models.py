...
def build_residual_shared_weights(time_length, input_channel, output_class_num, block_depth, dropout=0.5):
    inp = Input(shape=(time_length, input_channel), name="signal_input")
    condition_inp = Input(shape=(2,), name="condition_input")
    condition_embedding = RepeatVector(time_length)((Dense(2, name="condition_embedding")(condition_inp)))
    ...
    model_class = Model([inp,condition_inp],out_class)
    model_rul = Model([inp,condition_inp],out_rul)

    model_class.compile("adam",loss="categorical_crossentropy",metrics=["acc"])
    model_rul.compile("adam",loss="logcosh",metrics=["mse","mae"])
    return model_class, model_rul
    # adam = Adam(lr=0.01)
    # model.compile(adam,
    #               loss={
    #                   "dense_class_5":"categorical_crossentropy",
